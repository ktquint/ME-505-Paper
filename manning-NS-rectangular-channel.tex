...
\begin{equation}
    \hat{u}(\hat{y}, \hat{z}) = \hat{y} - \frac{\hat{y}^2}{2} + \sum_{1}^{\infty}A_n\sin(\lambda_n \hat{y})\cosh(\frac{\lambda_n}{\alpha}(\hat{z}-\frac{1}{2}))
\end{equation}
...
We calculate the flow rate using both the Navier-Stokes derived equation and Manning's equation for various channel dimensions and slopes under laminar flow conditions.
