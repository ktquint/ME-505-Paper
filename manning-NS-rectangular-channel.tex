...
\begin{abstract}
    ...
    varying Reynold's Numbers. Assuming a smooth channel ($n$ = 0.01),
...
\item[$\alpha$] = {$h/b$} = Aspect ratio (dimensionless)
\end{description}
...
large canals with gravity-driven flows \cite{GioiaPhysRevLett}. It's popularity stems from
its simplicity and, allowing engineers to quickly estimate flow rates based on channel 
...
\begin{itemize}
    ...
    \item Flow is irrotational $\rightarrow \mathbf{u} \cdot \nabla \mathbf{u} = 0$.
\end{itemize}
...
\begin{enumerate}
    ...
    \item No slip at the bottom and sides:
    \begin{itemize}
        \item $u|_{y=0, z}=0$
        \item $u|_{y, z=0}=0$
        \item $u|_{y, z=b}=0$
    ...
    \item Symmetry at the free surface:
    \begin{itemize}
        \item $\frac{\partial u}{\partial y}|_{y=h,z} = 0$
    \end{itemize}

\end{enumerate}

These boundary conditions are illustrated in Figure~\ref{fig:channel-boundary-conditions}.
...
where 
\begin{equation}
A_n = \frac{-2}{\lambda_n^3 \cosh\left(\frac{\lambda_n}{2\alpha}\right)} \quad \text{and} \quad \lambda_n = (2n - 1) \frac{\pi}{2}
\end{equation}
...
We found that no $Re$ yielded similar discharge values in both
...
flowrates across tested $Re$ values, with small regions where the two flowrates are
equivalent. Table~\ref{tab:reynolds_analysis} summarizes the best
matches found for each $Re$, along with the corresponding aspect ratio
...
all tested $Re$ values. The area highlighted in green indicates the range where the comparisons have
...
Our analysis revealed the Navier-Stokes flowrates were  greater
than the Manning's flowrate for aspect ratios greater than about 45 and across all $Re$ values.
 The opposite was true for aspect ratios less than 17. Table~\ref{tab:global_alpha_range}
...
from Manning's equation across a range of $Re$ values (250 to 12,500). Our results indicated
